...
func DashboardController(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		...
		tmpl, err := template.ParseFiles("apk1/views/layouts/layout.html",
			"apk1/views/dashboard/index.html")
		if err != nil {
		...
		<tr>
			...
			if inpId == "" {
				...
				err := config.DBBerno.QueryRow(query).Scan(&totalData)
				if err != nil {
					w.Header().Set("Content-Type", "text/html")
				...
				if totalData == 0 {
					result, err := config.DBBerno.Exec(`INSERT INTO dpfdplnew.distributors (nama, status, aktif, cons) VALUES (?, ?, ?, ?)`, inpNama, inpStatus, inpAktif, inpCons)
				...
				if totalData == 0 {
					...
					result, err := config.DBBerno.Exec(query)
					if err != nil {
						w.Header().Set("Content-Type", "text/html")
